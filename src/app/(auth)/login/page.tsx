...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ erro?: string; mensagem?: string }> }) {
  ...
  return (
    <>
      ...
      {demoMode && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-5 text-amber-800">O Supabase ainda não está configurado. O botão abaixo abrirá o modo demonstração.</div>}
      <form action={loginAction} className="mt-7 space-y-4">
        <Field label="E-mail"><input name="email" type="email" required={!demoMode} defaultValue={demoMode ? "demo@nexocrm.com.br" : ""} placeholder="voce@empresa.com.br" className={inputClass} /></Field>
        <Field label="Senha"><input name="password" type="password" required={!demoMode} defaultValue={demoMode ? "demonstracao" : ""} placeholder="Sua senha" className={inputClass} /></Field>
        <button className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-brand text-sm font-bold text-white shadow-sm transition-colors hover:bg-brand-strong">{demoMode ? "Acessar demonstração" : "Entrar no CRM"}<ArrowRight className="size-4" /></button>
      </form>
